...
uint32_t malloc_phys(size_t size, uint8_t align, uint32_t *physical)
{
  ...
  {
      void *address = allocate(size, (uint8_t)align, kernel_heap);
      page_t *page = fetch_page((uint32_t)address, 0, kernel_directory);
      //allocate_frame( fetch_page((uint32_t)address, kernel_directory));
...
static void alter_heap_size(uint32_t new_size, heap_t *heap)
{
  ...
  if( new_size > current_size )
  {
    ...
    {
        // Allocate frame here
        page = fetch_page(heap->start_address+i, 1, kernel_directory);
        allocate_frame(page, (heap->role)?1:0, (heap->readable)?1:0);
        i += PAGE_SIZE;
    }
  }
  else
  {
    ...
    {
        // Free frame here
        page = fetch_page(heap->start_address+i, 0, kernel_directory);
...
heap_t *initialise_heap(uint32_t start, uint32_t end_addr, uint32_t max, uint8_t role, uint8_t read)
{
